                     ...
                     m_erosion(erosion), m_paramH(paramH), m_diffusionD(diffusionD)
{
    ...
    //construction indexes for microsites with mutants
    ...
    //construction indexes for microsites with residents
...
//2nd construction, 2 types
//Microstes are filled up from microsite called "numeroCaseInitiale" then around in spiral.
//Watch out! Same number of arguments than in construction 2 but not same order.
                     ...
                     m_erosion(erosion), m_paramH(paramH), m_diffusionD(diffusionD)
{
    ...
    //check number of microsites with mutants
    if (nbrCasesMutant > squareLength*squareLength) {cout << "You're not focused: Too many mutants!" << endl; abort();}

    //construction indexes of microsites with mutants
    int j(0), k(1), kbis(1), indiceSigne(1), indiceAdd(1);
    if (numeroCaseInitiale >= squareLength*squareLength) {cout << "NumeroCaseInitale too high" << endl; abort();}
    ...
    for (int l(1); l < nbrCasesMutant; l++)
    {
        ...
        if (casesMutant[l] < 0 || casesMutant[l] >= squareLength*squareLength) {cout << "problem with calculation of casesMutant" << endl; abort();}
    }
    ...
    if (k > squareLength*squareLength-nbrCasesMutant)  {cout << "problem with calculation of casesResidents" << endl; abort();}
...
//3rd construction: initialization -> mutant microsites at the mutant phi value steady state, resident microsites at the resident phi value steady state.
        ...
        if (varUniform() < probaReplace)
        {
            ...
            for (int i(0); i<m_positions.size(); i++)
            {
                for (int i2(0); i2<m_positions[i].size(); i2++)
                {
                    ...
                    {
                        deathTab.push_back(i);  //type of individual
                        deathTab.push_back(i2); //index of individual in the table (that contains the individuals of the same strain)
                    }
                }
            }
                               ...
                               int const& indexInPop)
{
    ...
    if (indexSubPop >= m_individuals.size()) cout << "problem with function deleteMicrobe (2), wrong choice of strain to be deleted" << endl;
                                  ...
                                  double const& diffusionTimeStep, double const& probaReplace)
{
    ...
    if (m_sizeMTotal == 0) {cout << "problem: oneStep but there is no bacteria..."; abort();}
    timeStep = varExpo(microbeDeathRate*m_sizeMTotal); //gives the time before the next death
    if (timeStep < 0) {cout << "problem with calculation of timeStep"; abort();}
    ...
    if (timeStep >= diffusionTimeStep)
    {
        ...
        //watch out: the succession of the previous functions would create a problem if there are not exactly 2 strains
    ...
    else
    {
        ...
        if (va1 < m_microbeTypes[indexSubPop].deathRate())
        {
            ...
            //we empty microsite m_types[positionM] if there is no individuals in it
            bool vide(true);
